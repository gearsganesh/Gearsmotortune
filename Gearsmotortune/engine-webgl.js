/* Gearsmotortune Engine Background v5
 * 20-frame WebP sequence + two-texture GPU crossfade.
 * The engine stays behind the page content; hero content remains in front
 * at 90% opacity so the background is visible without washing out the text.
 */
(() => {
  'use strict';
  ...
  // The current Engine-frames directory contains frame_001_converted.webp through frame_020_converted.webp.
  ...
  const fragmentSource=`
    ...
    void main(){
      ...
      vec2 cover=vec2(1.0);
      if(ir>cr) cover.x=ir/cr; else cover.y=cr/ir;
      vec2 uv=(v_uv-.5)*cover/1.045+.5;
