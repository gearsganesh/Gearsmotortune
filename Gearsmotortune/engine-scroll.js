...
(() => {
  ...
  function addStyles() {
    ...
    style.textContent = `
      #${CANVAS_ID}{position:fixed;inset:0;width:100vw;height:100vh;z-index:0;display:block;pointer-events:none;background:#000}
      #${HERO_ID}{min-height:420vh!important;background:transparent!important;position:relative;isolation:isolate}
      ...
      .gmt-engine-vignette{position:fixed;inset:0;z-index:1;pointer-events:none;background:radial-gradient(ellipse at center,transparent 34%,rgba(0,0,0,.58) 100%)}
      ...
      @media(max-width:600px){#${HERO_ID}{min-height:360vh!important}#${HERO_ID} .hero-content{padding-top:76px!important}.gmt-engine-vignette{background:radial-gradient(ellipse at center,transparent 28%,rgba(0,0,0,.64) 100%)}}
  ...
  function createGL(canvas) {
    ...
    const fs = `precision mediump float; uniform sampler2D u_texture; uniform vec2 u_scale; varying vec2 v_uv; void main(){vec2 uv=(v_uv-.5)*u_scale+.5;gl_FragColor=texture2D(u_texture,uv);}`;
    ...
    const scaleLoc=gl.getUniformLocation(program,'u_scale');const texLoc=gl.getUniformLocation(program,'u_texture');gl.uniform1i(texLoc,0);
    ...
    function resize(){
      ...
      const viewportRatio=innerWidth/innerHeight;const imageRatio=frameW/frameH;let sx=1,sy=1;
      if(viewportRatio>imageRatio) sy=imageRatio/viewportRatio; else sx=viewportRatio/imageRatio;
      gl.useProgram(program);gl.uniform2f(scaleLoc,sx,sy);
    }
    function upload(image){
      frameW=image.naturalWidth||image.width||1920;frameH=image.naturalHeight||image.height||1080;
      gl.activeTexture(gl.TEXTURE0);gl.bindTexture(gl.TEXTURE_2D,texture);gl.pixelStorei(gl.UNPACK_FLIP_Y_WEBGL,true);
      gl.texImage2D(gl.TEXTURE_2D,0,gl.RGBA,gl.RGBA,gl.UNSIGNED_BYTE,image);resize();gl.drawArrays(gl.TRIANGLE_STRIP,0,4);
    }
    addEventListener('resize',resize,{passive:true});resize();return {upload};
  }

  async function preloadFrames(loader){
    const images=new Array(FRAME_COUNT);const fill=loader.querySelector('.gmt-loader-fill');const percent=loader.querySelector('.gmt-loader-percent');let loaded=0;
    const loadOne=(i)=>new Promise(resolve=>{
      const img=new Image();img.decoding='async';
      img.onload=()=>{images[i-1]=img;loaded++;const p=Math.round(loaded/FRAME_COUNT*100);fill.style.width=p+'%';percent.textContent=p+'%';resolve()};
      img.onerror=()=>{console.warn('Gearsmotortune: missing frame',i);loaded++;const p=Math.round(loaded/FRAME_COUNT*100);fill.style.width=p+'%';percent.textContent=p+'%';resolve()};
      img.src=FRAME_PATH(i);
    });
    const workers=Array.from({length:6},async(_,w)=>{for(let i=w+1;i<=FRAME_COUNT;i+=6) await loadOne(i)});
    await Promise.all(workers);return images.filter(Boolean);
  }

  async function init(){
    const hero=document.getElementById(HERO_ID);if(!hero)return;addStyles();const {canvas,loader}=createUI();
    try{
      ...
      const gl=createGL(canvas);if(!gl)throw new Error('WebGL unavailable');
      const frames=await preloadFrames(loader);if(!frames.length)throw new Error('No engine frames loaded');
      gl.upload(frames[0]);loader.classList.add('done');gsap.registerPlugin(ScrollTrigger);
      const state={frame:0};
      gsap.to(state,{frame:frames.length-1,ease:'none',scrollTrigger:{trigger:hero,start:'top top',end:'bottom bottom',scrub:0.7,invalidateOnRefresh:true},onUpdate:()=>{const index=Math.max(0,Math.min(frames.length-1,Math.round(state.frame)));gl.upload(frames[index])}});
